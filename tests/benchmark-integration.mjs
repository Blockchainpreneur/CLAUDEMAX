#!/usr/bin/env node
/**
 * CLAUDEMAX Integration Benchmark
 *
...
const WORKSPACE = join(HOME, '.claudemax', 'lightrag-workspace');
const PROMPT_CACHE = join(HOME, '.claudemax', 'prompt-cache');
const HELPERS = join(HOME, 'claudemax', 'helpers');
...
// LightRAG cache
const lrCacheDir = join(HOME, '.claudemax', 'lightrag-cache');
let lrCacheCount = 0;
try {
...
const hasMemoryBlock = typeof startOut === 'string' && startOut.includes('[CLAUDEMAX MEMORY]');
...
console.log(`\n  ${B}Recommendations${X}:`);
if (cacheMisses > 1) {
  console.log(`  ${Y}→${X} Run ${D}notebooklm login${X} to authenticate NLM, then ${D}node ~/claudemax/helpers/precompute-pipeline.mjs${X}`);
