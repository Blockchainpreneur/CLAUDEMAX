...
LOCK_FILE="/tmp/claudemax-tui.lock"
LOCAL_VERSION=$(cat "$DIR/VERSION" 2>/dev/null | tr -d '[:space:]')
VERSION_URL="https://raw.githubusercontent.com/Blockchainpreneur/CLAUDEMAX/main/VERSION"
UPDATE_CMD="cd ~/claudemax && git pull && bash install.sh"
...
if [ -f "$LOCK_FILE" ]; then
  ...
  if [ -n "$OLD_PID" ] && kill -0 "$OLD_PID" 2>/dev/null; then
    echo "CLAUDEMAX is already running (PID $OLD_PID)" >&2
    exit 0
  fi
  rm -f "$LOCK_FILE"
fi
