...
const STATE_DIR = join(HOME, ".claudemax");
...
async function writeContextFile(project: Project, summary: string | undefined): Promise<void> {
  ...
  const content = `# CLAUDEMAX Context — ${project.name}
  ...
  async fetch(req) {
    ...
    try {
      ...
      if (method === "POST" && path === "/session/start") {
        ...
        if (project) {
          ...
          await Bun.write(
            ...
            `# CLAUDEMAX Context — ${project.name}\nLast session: ${new Date().toISOString().split("T")[0]}\n${lastSession}\n`
          );
        }
      ...
      if (method === "GET" && path === "/context") {
        ...
        const lastSession = lastSummary || "First session — no prior context.";
        const context = `# CLAUDEMAX Context — ${project.name}\nLast session: ${new Date().toISOString().split("T")[0]}\n${lastSession}\n`;
...
console.log(`CLAUDEMAX daemon running on http://localhost:${PORT}`);
