#!/usr/bin/env node
/**
 * CLAUDEMAX — Task-specific CLAUDE.md segment generator
 *
 * Reads ~/.claude/CLAUDE.md, splits by ## headers, maps sections to task types,
 * and writes compressed subsets to ~/.claudemax/prompt-cache/claudemd-{taskType}.txt
 *
 ...
 * Output: ~/.claudemax/prompt-cache/claudemd-{taskType}.txt
 */
...
const TASK_SECTIONS = {
  'bug-fix': [
    ...ALWAYS_INCLUDE,
    'self-healing-workflows',
    'ripple-autopilot-engine-always-on',  // TOOLS protocol for tool selection
  ],
  'new-feature': [
    ...ALWAYS_INCLUDE,
    'ui-design-activate-only-when-building-ui',
