#!/usr/bin/env node
/**
 * CLAUDEMAX Pre-computation Pipeline
 *
...
const LIGHTRAG_CLI = join(HOME, 'claudemax', 'scripts', 'lightrag-cli.py');
const LIGHTRAG_WORKSPACE = join(HOME, '.claudemax', 'lightrag-workspace');
...
try {
  ...
  const bridgeScript = join(HOME, 'claudemax', 'helpers', 'lightrag-bridge.mjs');
...
try {
  ...
  if (raw.length > 50) {
    ...
    try {
      const masterFile = join(HOME, '.claudemax', 'nlm-cache', 'master-progress.md');
      let existing = '';
      ...
      try {
        ...
        const masterIdFile = join(HOME, '.claudemax', 'nlm-master-source-id');
        ...
        // Add updated master
        const result = execSync(
          `${NLM_BIN} source add "${masterFile}" --title "CLAUDEMAX Master Progress"`,
...
try {
  ...
  execSync(`node "${join(HOME, 'claudemax', 'helpers', 'intent-predictor.mjs')}"`, {
    encoding: 'utf8', timeout: 15000,
...
try {
  ...
  execSync(`node "${join(HOME, 'claudemax', 'helpers', 'claudemd-segments.mjs')}"`, {
