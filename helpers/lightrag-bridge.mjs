#!/usr/bin/env node
/**
 * CLAUDEMAX LightRAG Bridge — Node.js ↔ Python vector search
 *
...
const CLI_SCRIPT = join(HOME, 'claudemax', 'scripts', 'lightrag-cli.py');
const WORKSPACE = join(HOME, '.claudemax', 'lightrag-workspace');
const CACHE_DIR = join(HOME, '.claudemax', 'lightrag-cache');
 ...
 * Reads from ~/.claudemax/memory/ and ~/.claudemax/learnings/.
 */
export function ingestAllMemory() {
  const memoryDir = join(HOME, '.claudemax', 'memory');
  const learningsDir = join(HOME, '.claudemax', 'learnings');
  const entries = [];
