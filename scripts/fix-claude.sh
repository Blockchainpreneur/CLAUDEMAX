#!/bin/bash
# CLAUDEMAX — Fix Claude Desktop after updates
# Usage: bash ~/claudemax/scripts/fix-claude.sh

set -e
APP="/Applications/Claude.app"
ASAR="$APP/Contents/Resources/app.asar"
WORK="/tmp/claudemax-fix"
...
if ! grep -q "claudemaxInjectStyles" "$WORK/$MAIN_JS" 2>/dev/null; then
  echo "▸ Inyectando tema CLAUDEMAX..."
  cat >> "$WORK/$MAIN_JS" << 'JSEOF'

// CLAUDEMAX Black + Pastel — preload injection
;(function claudemaxInjectStyles() {
  var CSS = [
  ...
  function inject() {
    if (document.getElementById('claudemax-css')) return;
    var s = document.createElement('style');
    s.id = 'claudemax-css'; s.textContent = CSS;
